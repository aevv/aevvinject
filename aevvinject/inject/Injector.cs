...
#region pinvokes
        ...
        string currentDll;
        uint dllHandle;
        public int inject(int pid, string dllPath)
        {
            IntPtr hProcess = OpenProcess((int)(0x000F0000L | 0x00100000L | 0xFFF), false, pid);
            currentProcID = pid;
            return commonInject(hProcess, dllPath);
        }
        public int inject(string exePath, string dllPath)
        {
            ...
            currentProcID = lpProcessInfo.dwProcessId;
            return commonInject(hProcess, dllPath);
        }
        public int unject()
        {
            ...
            IntPtr hProcess = OpenProcess((int)(0x000F0000L | 0x00100000L | 0xFFF), false, currentProcID);
            ...
            IntPtr hThread = CreateRemoteThread(hProcess, new IntPtr(0), 0, loc, new IntPtr(dllHandle), 0, out x);
        ...
        private int commonInject(IntPtr hProcess, string dllPath)
        {
            currentDll = dllPath;
            if (hProcess == null || hProcess.ToInt32() == -1)
            {
